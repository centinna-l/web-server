const request = require("request");

// request({ url: url2, json: true }, (error, response) => {
//   if (error) {
//     log("Unable to connect to the Internet");
//   } else {
//     console.log("Longitude " + response.body.features[0].center[0]);
//     console.log("Latitude " + response.body.features[0].center[1]);
//   }
// });

const geocode = (address, callback) => {
  const url =
    "https://api.mapbox.com/geocoding/v5/mapbox.places/" +
    encodeURIComponent(address) +
    ".json?access_token=pk.eyJ1IjoiY2VudGlubmEtbCIsImEiOiJjanU0aGtsdnkweTEwNGRxZ2NxeGhwMjY4In0.6q8ty_2oY3Cg9NEB0s8eLQ&limit=1";
  request({ url: url, json: true }, (error, response) => {
    if (error) {
      callback("Unbale to connect to the Internet", undefined);
    } else if (response.body.features.length === 0) {
      callback("Unable to find Location", undefined);
    } else {
      callback(undefined, {
        latitude: response.body.features[0].center[1],
        longitude: response.body.features[0].center[0],
        location: response.body.features[0].place_name
      });
    }
  });
};

module.exports = geocode;
